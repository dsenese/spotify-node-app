 ...
 // your application requests refresh and access tokens
  ...
  if (state === null || state !== storedState) {
    ...
    var authOptions = {
      ...
      headers: {
        'Authorization': 'Basic ' + (new Buffer.alloc(process.env.CLIENT_ID + ':' + process.env.CLIENT_SECRET).toString('base64'))
      },
      json: true
    };
    ...
    request.post(authOptions, function(error, response, body) {
      if (!error && response.statusCode === 200) {
        ...
        var user = request.get(options, function(error, response, body) {
          console.log(body);
        //   return body;
          ctx.body = body
        });
...
router.get('/refresh_token', async (ctx) => {
    ...
    var authOptions = {
      url: 'https://accounts.spotify.com/api/token',
      headers: { 'Authorization': 'Basic ' + (new Buffer.alloc(process.env.CLIENT_ID + ':' + process.env.CLIENT_SECRET).toString('base64')) },
      form: {

...
router.get('/callback', async (ctx) => {
 // your application requests refresh and access tokens
  ...
  if (state === null || state !== storedState) {
    ...
    var [access_token, refresh_token, user, error] = await request.post(authOptions, async function(error, response, body) {
      if (!error && response.statusCode === 200) {
        ...
        return {'access_token': access_token, 'refresh_token': refresh_token, 'user': user, 'error': "success"};
      } 
      else {
        return {'access_token': null, 'refresh_token': null, 'user': null, 'error': 'invalid_token'};
      }
    });
    if (refresh_token && access_token && error != 'invalid_token') {
        ctx.query = {
            'access_token': access_token,
            'refresh_token': refresh_token,
            'user': user

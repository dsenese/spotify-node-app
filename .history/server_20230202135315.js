  ...
  if (state === null || state !== storedState) {
    ...
    var authResults = await request.post(authOptions, async function(error, response, body) {
      if (!error && response.statusCode === 200) {
        ...
        return {"access_token": access_token, "refresh_token": refresh_token, 'user': user, status:  "success"};
      } 
      else {
        return {'access_token': null, 'refresh_token': null, 'user': null, 'status': 'invalid_token'};
      }
    });
    await 

...
router.get('/callback', async (ctx) => {
 // your application requests refresh and access tokens
  ...
  var access_token = ctx.query.access_token || null;
  var refresh_token = ctx.query.refresh_token || null;
  ...
  if (state === null || state !== storedState) {
    ...
    await request.post(authOptions, async function(error, response, body) {
      if (!error && response.statusCode === 200) {
        ...
        console.log('Access Token1: ', access_token, 'refresh token1: ', refresh_token);
        ...
        //   })
        // );
        // JSON.parse({"access_token": access_token, "refresh_token": refresh_token, 'user': user, status:  "success"});
        
      } 
      
    //   else {
    //     JSON.parse({'access_token': null, 'refresh_token': null, 'user': null, 'status': 'invalid_token'});
    //   }
    });
    console.log('Access Token2: ', access_token, 'refresh token2: ', refresh_token);
    // console.log('Auth Results: ',);
//     if (authResults.refresh_token && authResults.access_token && authResults.error != 'invalid_token') {
//         ctx.query = {
//             'access_token': authResults.access_token,
//             'refresh_token': authResults.refresh_token,
//             'user': authResults.user
//         }
//         ctx.redirect('/ready'
//         +
//           querystring.stringify({

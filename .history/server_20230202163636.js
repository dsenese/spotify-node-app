...
router.get('/callback', async (ctx) => {
 // your application requests refresh and access tokens
  ...
  if (state === null || state !== storedState) {
    ...
    var r = return request.post(authOptions, async function(error, response, body, ctx) {
      if (!error && response.statusCode === 200) {
        ...
        // passTokens(access_token, refresh_token);
        // ctx.redirect('/#'
        // +
        //   querystring.stringify({
    ...
    var authResults = r.form();
    console.log("AUTH RESULTS", authResults);
    // ctx.redirect('/ready');
    // console.log('ctx.query********:', ctx.query.access_token, ctx.query.refresh_token, ctx.query.status);
    // if (ctx.query.access_token && ctx.query.refresh_token && ctx.query.status != 'invalid_token') {
    //     ctx.redirect('/ready'
    //     +
    //       querystring.stringify({
    //         access_token: ctx.query.access_token,

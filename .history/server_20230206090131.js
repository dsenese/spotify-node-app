...
const {sequelize, ar} = require('./models/spotify_db');
...
function saveTrackToDB(trackName, artistList, spotifyImageUri) {
    var track = new Track({
        trackName: trackName,
        artistList: artistList,
        spotifyImageUri: spotifyImageUri
    });
...
function parseTrackMetaData(body) {
    ...
    for (var i = 0; i < body.tracks.items[index].artists.length; i++) {
        artistList.push(body.tracks.items[index].artists[i].name);
    }

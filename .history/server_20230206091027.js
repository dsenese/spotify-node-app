...
function saveTrackToDB(isrc, trackName, artistList, spotifyImageUri) {
    sequelize.sync().then(() => {
...
function parseTrackMetaData(body) {
    ...
    for (var i = 0; i < body.tracks.items[index].artists.length; i++) {
        artistList.push({'': body.tracks.items[index].artists[i].id, 'name': body.tracks.items[index].artists[i].name});
    }

...
router.get('/isrc/:id', async (ctx) => {
    ...
    request.get(options, function(error, response, body) {
        ...
        var [trackName, artistList, spotifyImageUri, popularity] = parseTrackMetaData(body);
        saveTrackToDB(ctx.params.id, trackName, artistList, spotifyImageUri, popularity)
...
router.post('/searchTitle/:title', async (ctx) => {
    ...
    request.get(options, function(error, response, body) {
        ...
        // console.log('TRACK RESULTS', isrc, trackName, artistList, spotifyImageUri, popularity);
    })
    ctx.body = {
        action: 'successfully added track - ' + ctx.params.id
    }
...
function saveTrackToDB(isrc, trackName, artistList, spotifyImageUri, popularity) {
    console.log('TRACK RESULTS 2', trackName, artistList, typeof spotifyImageUri, spotifyImageUri, popularity);
    console.log(typeof spotifyImageUri);
    sequelize.sync().then(() => {     
        Track.create({
            isrc: isrc,
            name: trackName,
            spotify_image_uri: spotifyImageUri.toString(),
...
function parseTrackMetaData(body) {
    ...
    var spotifyImageUri = body.tracks.items[index].album.images[0].url.toString();

    console.log('TRACK RESULTS', trackName, artistList, typeof spotifyImageUri, spotifyImageUri, popularity);
    return [body.tracks.items[index]['id'], trackName, artistList, spotifyImageUri, popularity];
}

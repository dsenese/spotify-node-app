...
router.get('/getTrack/:id', async (ctx) => {
    var trackResults = await getTrack(ctx.params.id)
    ctx.body = trackResults;
});

router.get('/getTracksByArtist/:artist', async (ctx) => {
...
async function getTrack(isrc) {
    ...
    var result = {
        isrc: isrc,
        name: track[].name,
        shopify_image_uri: track.shopify_image_uri,
        artists: artists
    }
    return result;
}

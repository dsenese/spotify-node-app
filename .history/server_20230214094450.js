...
router.get('/getTrack/:isrc', async (ctx) => {
    var trackResults = await getTrack(ctx.params.isrc)
...
router.get('/getTracksByArtist', async (ctx) => {
    const tracks = await Track.findAll({
        include: {
            model: Artist,
            where: { name: 'YOUR_ARTIST_NAME_HERE' }
        }
    });
...
async function saveTrackToDB(isrc, trackName, artistList, spotifyImageUri, popularity) {
    ...
    if (!isTrack) {
        ...
        for (var x = 0; x < artistList.length; x++) {
            ...
            if (!isArtist) {
                newArtists.append(await Artist.create({
                        id: artistList[x].id,
                ...
                newArtists.append(isArtist);
            }
        }
        newTrack.addArtist(newArtists);
    }
...
async function getTrack(isrc) {
    ...
    return {error: 'Failed to get track.'};
}

...
router.post('/searchTitle', async (ctx) => {
    var access_token = localStorage.getItem('access_token');
    var refresh_token = localStorage.getItem('refresh_token');
    var {title} = ctx.request.body;
    console.log('ISRC CALLED')
    var statusMsg = ''
    var options = {
        url: 'https://api.spotify.com/v1/search?q=title:' + title + '&type=track',
        headers: { 'Authorization': 'Bearer ' + access_token },
        json: true
    }
    request.get(options, function(error, response, body) {
...
async function saveTrackToDB(isrc, trackName, artistList, spotifyImageUri, popularity) {
    ...
    if (!isTrack) {
        ...
        statusMsg = `New track ${isTrack.name} was successfully added to db with isrc ${isTrack.isrc}`;

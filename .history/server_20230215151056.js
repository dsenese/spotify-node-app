...
router.get('/isrc/:id', async (ctx) => {
    ...
    request.get(options, async function(error, response, body) {
        ...
        if (body.error) {
            ...
            console.log(body);
            var [isrc, trackName, artistList, spotifyImageUri, popularity] = await parseTrackMetaData(body);
...
router.post('/searchTitle', async (ctx) => {
    ...
    statusMsg = await request.get(options, function(error, response, body) {
        //save to db
        if (body.error) {
            console.log(body.error);
            statusMsg = body.error;

...
router.get('/isrc/:id', async (ctx) => {
    ...
    var statusMsg = await request.get(options, function(error, response, body) {
        //save to db
        console.log('BODY', typeof body.tracks.items)
        if (body.error) {
            ...
            statusMsg = "Did not find track from isrc - " + ctx.params.title
        }
        // ctx.body = {
        //     statusMsg: statusMsg
        // }
    ...
    console.log('RES', await statusMsg.json().body)
    ctx.body = {
        statusMsg: await statusMsg.json()
    }

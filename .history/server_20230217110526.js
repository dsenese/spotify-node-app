...
app.use(koaSwagger.koa({
  ...
  app.use(async (ctx) => {
    ...
    } else if (ctx.request.path === '/posts') {
      ctx.body = { message: 'List of posts' };
    } else {
  ...
  app.listen(3000);
  In the example above, the swaggerOptions object defines the base Swagger specification, and the apis property specifies the files to search for JSDoc annotations. The swaggerJSDoc function generates the Swagger specification file from the annotations in app.js. The koaSwagger middleware serves the Swagger UI, and the API endpoints are defined in the Koa application using the standard app.use method.
  
  While generating a Swagger/OpenAPI specification file automatically can save you some time and effort, it's important to note that you still need to review and validate the generated file to ensure that it accurately represents your API's behavior and requirements.
...
router.get('/swagger', async (ctx) => {
    ctx.body = swaggerUi.generateHTML(spec);
});
